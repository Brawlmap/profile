...
# Comma-separated origins, or * for local dev only. Example: https://brawlmap.gg,https://www.brawlmap.gg
_allowed_raw = (os.getenv("ALLOWED_ORIGIN") or "*").strip()
ALLOWED_ORIGINS = [o.strip() for o in _allowed_raw.split(",") if o.strip()] or ["*"]
ENABLE_LEGACY_CMS = os.getenv("ENABLE_LEGACY_CMS", "").lower() in ("1", "true", "yes")
ENABLE_DEBUG = os.getenv("ENABLE_DEBUG", "").lower() in ("1", "true", "yes")

def _cors_origin():
    origin = request.headers.get("Origin")
    if not origin:
        return ALLOWED_ORIGINS[0] if len(ALLOWED_ORIGINS) == 1 else None
...
def _require_api_key():
    if not API_KEY:
        return jsonify({"error": "BRAWL_STARS_API_KEY is not configured on the server."}), 503
    return None

# Legacy JSON-file CMS (optional — site uses Firebase + Supabase by default)
ADMIN_PASSWORD = os.getenv("CMS_PASSWORD") if ENABLE_LEGACY_CMS else None
if ENABLE_LEGACY_CMS and not ADMIN_PASSWORD:
    raise RuntimeError("ENABLE_LEGACY_CMS is set but CMS_PASSWORD is missing.")
...
def _legacy_cms_disabled():
    return jsonify({"error": "Legacy CMS is disabled on this server."}), 404

def require_auth():
    """Returns None if ok, or a JSON error response tuple."""
    if not ENABLE_LEGACY_CMS:
        return _legacy_cms_disabled()
...
# ── CMS: Posts (legacy JSON — disabled unless ENABLE_LEGACY_CMS=true) ───────
